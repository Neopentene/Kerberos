...
class DatabaseManager:
    ...
    def check_user(self, username: str):
        if self.check_connection():
            query = "SELECT username FROM dbo.users WHERE username = ?"
            with self.db.cursor() as cursor:
                try:
                    cursor.execute(query, [username])
                    value = cursor.fetchone()
                    user = value[0] if value is not None else None
    ...
    def get_tgs_session_key(self, username: str):
        if self.check_connection():
            query = "SELECT session_key FROM dbo.tgs_sessions WHERE username = ? AND valid_till >= GETDATE()"
            with self.db.cursor() as cursor:
                try:
                    cursor.execute(query, [username])
                    value = cursor.fetchone()
    ...
    def _delete_old_session(self, username: str):
        if self.check_connection():
            query = "DELETE FROM dbo.tgs_sessions WHERE username = ?"
            with self.db.cursor() as cursor:
                try:
                    cursor.execute(query, [username])
                    return True
    ...
    def create_new_tgs_session(self, username: str, session_key: str):
        if self.check_connection():
            print("here-create-tgs")
            self._delete_old_session(username)
            self.commit()
            query = "INSERT INTO dbo.tgs_sessions VALUES (?, ?, GETDATE(), DATEADD(DAY, 1, GETDATE()))"
            with self.db.cursor() as cursor:
                try:
